...
def admin_page():
    ...
    if current_role in allowed_roles:
        ...
        # Overview of loaded datasets
        loaded_data = importer.get_loaded_datasets()
        if not loaded_data.empty:
            st.subheader("Loaded Datasets Overview")
            # Select and rename columns for a nicer display
            display_cols = {
            ...
            st.dataframe(
                loaded_data[list(display_cols.keys())].rename(columns=display_cols),
                hide_index=True,
                width='stretch'
            )
