        ...
        with tab_addr:
            ...
            with col_info:
                ...
                edit_mode_active = st.toggle(
                    "✏️ Edit Address Location",
                    value=st.session_state.get('edit_addr_mode', False),
                    help="Activating this switch allows you to point to a new location on the map to reposition the address marker.",
                    key="edit_addr_mode_toggle"
                )
                
                # Sync session state with key
            ...
            with col_map:
                if st.session_state.get('pending_geocode') and folium and st_folium:
                    ...
                    if st.button("📍 Confirm Geocoded Location", type="primary", use_container_width=True):
                        # DB logic later
                        pass
                    ...
                    if geom:
                        ...
                        if is_draft:
                            folium.Marker(location=[geom['latitude'], geom['longitude']], tooltip="Original", icon=folium.Icon(color='gray', icon='home', opacity=0.5)).add_to(m)
                            folium.Marker(location=[display_lat, display_lng], tooltip="New (Unsaved)", icon=folium.Icon(color='orange', icon='info-sign')).add_to(m)
                        else:
                            folium.Marker(location=[geom['latitude'], geom['longitude']], tooltip=a_dict.get('ADDR_LINE1', ''), icon=folium.Icon(color='green', icon='home')).add_to(m)
                        ...
                        if edit_mode_active and map_output.get("last_clicked"):
                            st.session_state.draft_addr_location = (map_output["last_clicked"]["lat"], map_output["last_clicked"]["lng"])
                            st.rerun()
                else:
                ...
                if edit_mode_active:
                    ...
                    with c1:
                        if st.button("💾 Save Position", type="primary", disabled=not has_draft, use_container_width=True):
                            # DB logic later
                            pass
                    with c2:
                        if st.button("🔄 Reset", disabled=not has_draft, use_container_width=True):
                            st.session_state.draft_addr_location = None
                            st.rerun()
            ...
            if submitted:
                if update_property_address(selected_id, new_line1, new_line2,
                            ...
                            new_addr_type):  # Adding new_addr_type
